...
namespace FreeNet
{
    ...
    public class CPacket
    {
        ...
        public static CPacket[] split(CPacket target)
        {
            CPacket[] result = new CPacket[(int)Math.Ceiling(target.buffer.Length / 1024f)];
            int position = target.position;

            for(int i = 0; i < result.Length; i++)
            {
                result[i] = new CPacket();
            }

            for (int i = 0; i < target.buffer.Length; i += 1024)
            {
                Array.Copy(target.buffer, i, result[i / 1024].buffer, 0, 1024);
                result[i / 1024].position = Clamp(0, 1024, position);
                position -= 1024;
            }
        ...
        public float pop_Single()
        {
            ...
            //return data;
            float data = float.Parse(pop_string());
            return data;
        }
        ...
        public void push_int16(Int16 data)
        {
            ...
            CopyTo(ref temp_buffer, ref this.buffer);
        ...
        public void push_int32(Int32 data)
        {
            byte[] temp_buffer = BitConverter.GetBytes(data);
            //temp_buffer.CopyTo(this.buffer, this.position);
            CopyTo(ref temp_buffer, ref this.buffer);
        ...
        public void push(string data)
        {
            ...
            CopyTo(ref len_buffer, ref this.buffer);
            this.position += sizeof(Int16);
        ...
        private void CopyTo(ref byte[] from, ref byte[] to)
        {
            while(true)
            {
                if(this.position + from.Length > to.Length)
                {
                    byte[] temp_buffer = new byte[to.Length + 1024];

                    to.CopyTo(temp_buffer, 0);
                    to = temp_buffer;
                }
                else
                {
